...
char *bump_alloc(BumpAllocator *bump_allocator, size_t size) {
    char *result = nullptr;
    size_t allignedSize = (size + 7) & ~7; // -> First 4 bits are 0.
    if (bump_allocator->used + allignedSize <= bump_allocator->capacity) {
        result = bump_allocator->memory + bump_allocator->used;
        bump_allocator->used += allignedSize;
    } else {
...
bool copy_file(char *fileName, const char *outputName, BumpAllocator *bumpAllocator) {
    char *file = 0;
